...
const latestTunnelVersion = "v0.38.2"
const tmpPath = "/tmp/"

async function run() {

    console.log("Preparing output location...")
    const outputPath = tmpPath + "tunnel-results-" + Math.random() + ".json";
    ...
    const hasAccount = hasKhulnasoftAccount()
    const assurancePath = tmpPath + "tunnel-assurance-" + Math.random() + ".json";

    if(hasAccount) {
        task.rmRF(assurancePath);
        const credentials = getKhulnasoftAccount()
        process.env.AQUA_KEY = credentials.key
        process.env.AQUA_SECRET = credentials.secret
        process.env.TUNNEL_RUN_AS_PLUGIN = "aqua"
    ...
    console.log("Running Tunnel...")
    let result = runner.execSync();
    if (result.code === 0) {
        task.setResult(task.TaskResult.Succeeded, "No problems found.")
    } else {
        task.setResult(task.TaskResult.Failed, "Failed: Tunnel detected problems.")
    }

    if(hasAccount) {
        console.log("Publishing JSON assurance results...")
        task.addAttachment("ASSURANCE_RESULT", "tunnel-assurance-" + Math.random() + ".json", assurancePath)
    }

    console.log("Publishing JSON results...")
    task.addAttachment("JSON_RESULT", "tunnel" +  Math.random() + ".json", outputPath)
    console.log("Done!");
}
...
function hasKhulnasoftAccount(): boolean {
    const credentials = getKhulnasoftAccount()
...
function getKhulnasoftAccount(): aquaCredentials {
...
async function createRunner(docker: boolean, loginDockerConfig: boolean): Promise<ToolRunner> {
    ...
    if (!docker) {
        console.log("Run requested using local Tunnel binary...")
        const tunnelPath = await installTunnel(version)
        return task.tool(tunnelPath);
    }
    ...
    if(hasKhulnasoftAccount()) {
        runner.line("-e TUNNEL_RUN_AS_PLUGIN")
        ...
        if (isDevMode()) {
            runner.line("-e AQUA_URL=https://api-dev.khulnasoft.com/v2/build")
            runner.line("-e CSPM_URL=https://stage.api.cloudsploit.com/v2/tokens")
        }
    }
    runner.line("khulnasoft/tunnel:" + stripV(version))
    return runner
}
...
async function installTunnel(version: string): Promise<string> {

    console.log("Finding correct Tunnel version to install...")
    ...
    console.log("Downloading Tunnel...")
    let downloadPath = await tool.downloadTool(url, localPath);

    console.log("Extracting Tunnel...")
...
async function getArtifactURL(version: string): Promise<string> {
    if(version === "latest") {
        version = latestTunnelVersion
    }
    console.log("Required Tunnel version is " + version)
    let arch = ""
    switch (os.arch()) {
        case "arm":
            arch = "ARM"
    ...
    // e.g. tunnel_0.29.1_Linux-ARM.tar.gz
    let artifact: string = util.format("tunnel_%s_Linux-%s.tar.gz", stripV(version), arch);
    return util.format("https://github.com/khulnasoft/tunnel/releases/download/%s/%s", version, artifact);
}
