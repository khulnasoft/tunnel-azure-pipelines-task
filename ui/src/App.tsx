...
import {Report, AssuranceReport} from './tunnel'
...
export class App extends React.Component<AppProps, AppState> {
    ...
    async check() {
        ...
        timeline.records.forEach(function (record: TimelineRecord) {
            if (record.type == "Task" && record.task !== null && record.task.name == "tunnel") {
        ...
        if (attachments.length === 0) {
            this.setState({error: "No attachments found: cannot load results. Did Tunnel run properly?"})
            return
        }
